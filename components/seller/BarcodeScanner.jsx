  ...
  try {
    ...
    try {
      ...
      if (result && result.getText()) {
        ...
        // Prevent duplicate scans (reduced to 1 second to allow faster re-scanning)
        if (now - lastScanTimeRef.current > 1000 && barcodeText) {
          lastScanTimeRef.current = now;
          console.log('Barcode detected:', barcodeText);
          
          // Call the callback - scanner remains active for continuous scanning
          ...
          <div className="relative w-11/12 max-w-sm h-48">
            ...
            <div className="absolute -bottom-8 left-0 right-0 text-center">
              <p className="text-white text-sm bg-black bg-opacity-50 px-3 py-1 rounded">
                Point camera at barcode - Scanner stays active
              </p>
            </div>
          </div>
        </div>
